...
export function MonthCalendar({ mealDays, workoutDays, onNavigate }: {
  mealDays: string[]; workoutDays: string[]; onNavigate?: () => void;
}) {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: "4px" }}>
        {cells.map((day, i) => {
          ...
          return (
            <button
              key={dateStr}
              onClick={() => { if (!isFuture) { onNavigate?.(); router.push(`/progress/calendar?date=${dateStr}`); } }}
              disabled={isFuture}
              style={{
